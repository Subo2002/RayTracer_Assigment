...
public class Renderer {
	...
	//How many shadow rays a sent out for each lightsource
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O) {
		...
		for (int i = 0; i < pointLights.size(); i++) {
			...
			Vector3 L_ = L.add(Vector3.randomInsideUnitSphere());
			Ray shadowRay = new Ray(P.add(N.scale(EPSILON)), L_);
			...
			if (scene.findClosestIntersection(shadowRay).getDistance() > distanceToLight) {
				...
				// TODO: Calculate L, V, R (The Light Source, The View(whether it be from a camera or next reflection), the Reflection in the normal of the lightsource ont he surface)
				L = L.normalised();
				...
				ColorRGB diffuse = I.scale(k_d * Math.max(0, N.dot(L))).scale(C_diff);
				ColorRGB specular = I.scale(k_s * Math.pow(Math.max(0, R.dot(V)), alpha)).scale(C_spec);
